        ...
        <div className="flex flex-col justify-between min-h-screen bg-[radial-gradient(circle_at_top,_rgba(251,191,36,0.14),_transparent_40%),linear-gradient(180deg,_#0f172a_0%,_#020617_100%)] px-3 py-8 text-slate-100 sm:px-5 lg:px-6">
            ...
            <div className="mx-auto flex w-full max-w-3l flex-col gap-5 rounded-3xl border border-white/10 bg-white/5 p-4 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-6">
                ...
                <div
                    ...
                    className="group flex cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-white/15 bg-slate-950/40 px-4 py-6 text-center transition-all duration-300 hover:border-amber-400 hover:bg-white/10 hover:shadow-[0_0_0_1px_rgba(251,191,36,0.18)] focus:outline-none focus:ring-2 focus:ring-amber-400/60 focus:ring-offset-0 sm:px-6"
                    aria-label="Drop image here or click to select"
                >
                    {preview && previewShow ? (

                        <div className="space-y-2 text-center text-slate-300">
                            <img src={preview.url} alt="preview" className="mx-auto h-56 w-56 rounded-2xl object-cover ring-1 ring-white/10 shadow-lg shadow-black/40 sm:h-64 sm:w-64" />
                ...
                <div className="m-auto flex  gap-2 flex-row flex-wrap">
                    ...
                    <button onClick={removeImage} className="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 font-medium text-slate-100 transition-colors duration-200 hover:border-white/20 hover:bg-white/10">
                        Remove
                    </button>
                </div>
                ...
                {result && <div className="mt-1 rounded-3xl border border-white/10 bg-slate-950/60 p-4 shadow-xl shadow-black/30 backdrop-blur sm:p-5">
                    <h1 className="flex justify-around text-lg font-semibold text-white">Grain Details</h1>
                    <div className='mt-3 flex flex-col gap-4 md:flex-row md:items-start md:gap-5' >
...
//             const response = await fetch("https://get-grain-details.onrender.com/grain-info", {
//                 method: "POST",
//                 headers: {
//                     "Content-Type": "application/json"
...
//                 },
//                 body: JSON.stringify({
//                     grain_name: data.predicted_label
//                 })
//             });

//             // IMPORTANT: only read the response body ONCE.
//             // Calling response.json() a second time throws
//             // "body stream already read" and silently breaks grainInfo.
//             const grain_info = await response.json();

//             if (!response.ok) {
//                 console.error("API error:", grain_info.detail);
//                 setError(grain_info.detail || `Grain info request failed: ${response.status}`);
//             } else {
//                 console.log(grain_info);
//                 setgrainInfo(grain_info);
//                 setpreviewShow(false);
